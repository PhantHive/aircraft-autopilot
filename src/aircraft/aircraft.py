...
class AircraftStability(Params):
    ...
    def __init__(self):
        ...
        super().__init__()
        # ------------------- Initialization -------------------
        self.Vsound = sqrt(self.p['gamma']['value'] * self.p['R']['value'] * self.p['T0']['value'])
        print("Vsound = ", self.Vsound)
        self.Veq = self.p['Mach']['value'] * self.Vsound
...
class StateSpaceModel(Params):
    ...
    def __init__(self, aircraft: AircraftStability):
        ...
        self.Cx_alpha = 2 * self.p['k']['value'] * self.Cz * self.p['Cz_alpha']['value']
        self.F_tau = 0
        self.Cm_alpha = (self.aircraft.X / self.p['lref']['value']) * (
                    self.Cx_alpha * math.sin(self.aircraft.alpha_eq) + self.p['Cz_alpha']['value'] * math.cos(
                self.aircraft.alpha_eq))
        self.Cm_delta_m = (self.aircraft.Y / self.p['lref']['value']) * (
                    self.aircraft.Cx_delta_m * math.sin(self.aircraft.alpha_eq) + self.p['Cz_delta_m'][
                'value'] * math.cos(self.aircraft.alpha_eq))

        # ------------------- State space model final parameters of our matrices -------------------
        self.Xv = (2 * self.aircraft.Q * self.p['S']['value'] * self.aircraft.Cx_eq) / (
                    self.p['m']['value'] * self.aircraft.Veq)
        self.Xalpha = (self.aircraft.Fpx_eq / (self.p['m']['value'] * self.aircraft.Veq)) * math.sin(
            self.aircraft.alpha_eq) + (self.aircraft.Q * self.p['S']['value'] * self.Cx_alpha) / (
                                  self.p['m']['value'] * self.aircraft.Veq)
        self.Xgamma = self.p['g']['value'] * math.cos(self.gamma_eq) / self.aircraft.Veq
        self.Xdelta_m = (self.aircraft.Q * self.p['S']['value'] * self.aircraft.Cx_delta_m) / (
                    self.p['m']['value'] * self.aircraft.Veq)
        self.Xtau = - (self.F_tau * math.cos(self.aircraft.alpha_eq)) / (
                    self.aircraft.p['m']['value'] * self.aircraft.Veq)
        self.mv = 0
        self.m_alpha = (self.aircraft.Q * self.p['S']['value'] * self.p['lref']['value'] * self.Cm_alpha) / self.Iyy
        self.m_q = (self.aircraft.Q * self.p['S']['value'] * (self.p['lref']['value'] ** 2) * self.p['Cm_q'][
            'value']) / (self.aircraft.Veq * self.Iyy)
        self.m_delta_m = (self.aircraft.Q * self.p['S']['value'] * self.p['lref']['value'] * self.Cm_delta_m) / self.Iyy
        self.Zv = (2 * self.aircraft.Q * self.p['S']['value'] * self.aircraft.Cz_eq) / (
                    self.p['m']['value'] * self.aircraft.Veq)
        self.Zalpha = (self.aircraft.Fpx_eq * math.cos(self.aircraft.alpha_eq)) / (
                    self.p['m']['value'] * self.aircraft.Veq) + (
                                  self.aircraft.Q * self.p['S']['value'] * self.p['Cz_alpha']['value']) / (
                                  self.p['m']['value'] * self.aircraft.Veq)
        self.Zgamma = (self.p['g']['value'] * math.sin(self.gamma_eq)) / self.aircraft.Veq
        self.Zdelta_m = (self.aircraft.Q * self.p['S']['value'] * self.p['Cz_delta_m']['value']) / (
                    self.p['m']['value'] * self.aircraft.Veq)
